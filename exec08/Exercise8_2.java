...
public class Exercise8_2 {
	...
	public static void main(String[] args) {
		...
		// タンク容量を受け付け
		double carTankCapacity = standardInput.nextDouble();
		// タンク容量が0以下の場合
		if (carTankCapacity <= ZERO_NUMBER) {
			// 後判定ループ文
			do {
				// 正の実数値の入力を促す
				System.out.print("正の実数を入力してください：");
				// タンク容量を受け付け
				carTankCapacity = standardInput.nextDouble();
				// 正の実数値の入力で抜け出す
			} while (carTankCapacity <= ZERO_NUMBER);
		}
		// 燃料がタンク容量を超える場合
		if (carFuel > carTankCapacity) {
			// 燃料をタンク容量に設定
			carFuel = carTankCapacity;
		}
